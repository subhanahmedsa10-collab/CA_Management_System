...
// Schema is embedded so it works in dev, packaged, and forked-child contexts
// (avoids any asar/path issues with reading schema.sql)
...
// Health check — useful for diagnosing if backend is reachable
app.get('/api/ping', (req, res) => {
  try {
    const userCount = rawDb.prepare('SELECT COUNT(*) AS c FROM users').get().c;
    res.json({ ok: true, dbPath, users: userCount });
  } catch (err) {
...
// Surface uncaught crashes so they show up in the log file
